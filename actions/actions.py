...
class ActionFetchYoutubeVideos(Action):
    ...
    def __init__(self):
        ...
        self.youtube = build('youtube', 'v3', developerKey='AIzaSyAorLr22WUfhsZ-nMpEvERDujtb9WJyUY0')
...
class SolveMathProblem(Action):
    def name(self) -> Text:
        return "action_solve_math_problem"
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        #math_expression = re.sub(r"^(?:calculate|solve|what is|find the value of|what's|can you solve|can you find|find)\s+", "", user_input) 
        #math_expression = re.sub(r"[\s,;]+", "", math_expression)  # Remove extra spaces, commas, and semicolons


        try:
            from sympy import sympify, solve
            expr = sympify(user_input)
            solution = f"The solution to {user_input} is: {expr}"
        except Exception as e:
            solution = f"I couldn't solve that problem. Please make sure the problem is correctly formatted. Error: {e}"
...
class ActionFetchData(Action):
    ...
    async def run(self, dispatcher: CollectingDispatcher,
        ...
        dispatcher.utter_message(text="Here is the data you requested.")
        return []
